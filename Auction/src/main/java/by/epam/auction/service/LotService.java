		...
		try (BidDAOImpl bidDAO = new BidDAOImpl()) {
			...
			if(checkBidValidity(lot, bidder, owner, amount)) {
				...
				if(lot.getAuctionType() == AuctionType.DIRECT) {
					...
					if(previousBidderOptional.isPresent() && previousBidderOptional.get().getId() != owner.getId()) {
		...
		try (BidDAOImpl bidDAO = new BidDAOImpl()) {
			...
			if (optionalUser.isPresent() && !optionalUser.get().isBlocked()) {
				...
				if (optionalAuction.isPresent() && optionalAuction.get().getFinishTime().isAfter(LocalDateTime.now())
						&& !(optionalAuction.get().getType() == AuctionType.REVERSE
						&& optionalUser.get().getMoney().compareTo(amount)<0)) {
					...
					if (categoryMap != null && categoryMap.keySet().contains(categoryId)) {
						...
						for (Part picture : pictureSet) {
							String fileName = new ImageSaver().writeImg(picture, path);
							if(!fileName.isEmpty()) {
								pictureNameSet.add(fileName);
							} else {
						...
						if(optionalAuction.get().getType() == AuctionType.REVERSE) {
							User owner = optionalUser.get();
							owner.setFrozenMoney(owner.getFrozenMoney().add(amount));
							owner.setMoney(owner.getMoney().subtract(amount));
							bidDAO.updateUser(owner);
						}
